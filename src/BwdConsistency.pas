...
function NarrowToInterval(VIdx, OpNum: Integer; Target: TInterval;
                          const Expr: string): Boolean;
{ Estrecha el dominio de VIdx a la intersección con Target }
begin
  Result := False;
  if IvIsEmpty(Target) <> 0 then begin MarkEmpty(VIdx, OpNum, Expr); Result := True; Exit; end;
  Result := NarrowLo(VIdx, OpNum, Target.Lo, Expr) or Result;
  if not Vars[VIdx].Empty then
    Result := NarrowHi(VIdx, OpNum, Target.Hi, Expr) or Result;
end;
...
// ── Evaluación de intervalo (forward, para calcular subexpresiones) ───────────
...
// ── Proyección inversa a través del árbol de expresiones ──────────────────────
//
//  Node   : nodo del árbol que contiene (posiblemente) la variable VarIdx
//  VarIdx : variable a estrechar
//  Target : el intervalo que el resultado de Node debe satisfacer
//  OpNum  : número de operación de cola (para trazado)
//  Expr   : cadena de la constraint (para trazado)
//
//  La función estrecha VarIdx recursivamente aplicando las reglas:
//    −A = T        → A ∈ −T
//    A + B = T     → A ∈ T − B,  B ∈ T − A
//    A − B = T     → A ∈ T + B,  B ∈ A − T
//    A * B = T     → A ∈ T / B,  B ∈ T / A
...
function BackProjectNode(Node: TASTNode; VarIdx, OpNum: Integer;
                         Target: TInterval; const Expr: string): Boolean; forward;

function BackProjectNode(Node: TASTNode; VarIdx, OpNum: Integer;
                         Target: TInterval; const Expr: string): Boolean;
var
  AVid:         Integer;
  AIv, BIv:    TInterval;
  Left, Right: TASTNode;
begin
  ...
  if IvIsEmpty(Target) <> 0 then Exit;   { Nada que propagar si el objetivo está vacío }

  case Node.NodeType of

    ntVariable:
    begin
      AVid := FindVar(TVariableNode(Node).Name);
      if (AVid >= 0) and (AVid = VarIdx) and (Vars[AVid].Kind = vkNumeric) then
        Result := NarrowToInterval(AVid, OpNum, Target, Expr);
    end;

    ntNegate:
    begin
      { −A = T  →  A = −T }
      Result := BackProjectNode(TUnaryOpNode(Node).Operand, VarIdx, OpNum,
                                IvScale(Target, -1.0), Expr);
    end;

    ntAdd:
    begin
      { A + B = T  →  A = T − B,  B = T − A }
      Left  := TBinaryOpNode(Node).Left;
      Right := TBinaryOpNode(Node).Right;
      AIv := EvalIv(Left);
      BIv := EvalIv(Right);
      Result := BackProjectNode(Left,  VarIdx, OpNum, IvSub(Target, BIv), Expr) or Result;
      Result := BackProjectNode(Right, VarIdx, OpNum, IvSub(Target, AIv), Expr) or Result;
    end;
    ...
    begin
      ...
      AIv := EvalIv(Left);
      BIv := EvalIv(Right);
      Result := BackProjectNode(Left,  VarIdx, OpNum, IvAdd(Target, BIv), Expr) or Result;
      Result := BackProjectNode(Right, VarIdx, OpNum, IvSub(AIv, Target), Expr) or Result;
    end;
    ...
    begin
      ...
      AIv := EvalIv(Left);
      BIv := EvalIv(Right);
      Result := BackProjectNode(Left,  VarIdx, OpNum, IvDiv(Target, BIv), Expr) or Result;
      Result := BackProjectNode(Right, VarIdx, OpNum, IvDiv(Target, AIv), Expr) or Result;
    end;
    ...
    begin
      ...
      AIv := EvalIv(Left);
      BIv := EvalIv(Right);
      Result := BackProjectNode(Left,  VarIdx, OpNum, IvMul(Target, BIv), Expr) or Result;
      Result := BackProjectNode(Right, VarIdx, OpNum, IvDiv(AIv, Target), Expr) or Result;
    end;

  { Números, booleanos: no hay variable que estrechar }
  end;
end;

// ── Procesamiento de un arco — consistencia hacia atrás ──────────────────────
  ...
  try
    try
      ...
      try
        ...
        { ── Igualdad numérica: proyección inversa completa ── }
        if Op = ntEquals then
        begin
          LIv := EvalIv(LHS);
          RIv := EvalIv(RHS);

          { Back-project VarIdx a través de RHS usando el dominio de LHS }
          Result := BackProjectNode(RHS, VarIdx, OpNum, LIv, Expr) or Result;

          { Back-project VarIdx a través de LHS usando el dominio de RHS }
          Result := BackProjectNode(LHS, VarIdx, OpNum, RIv, Expr) or Result;

          Exit;
        end;

        { ── Comparaciones: narrowing directo (igual que forward) ── }
        LIv := EvalIv(LHS);
        RIv := EvalIv(RHS);
...
begin
  ...
  begin
    ...
    SObj.AddStr('arc',        Steps[i].VarName + ' <- ' + Steps[i].CstrExpr + ' (bwd)');
...
begin
  ...
  try
    ...
    { Cola inicial en orden INVERSO: última constraint primero }
    for c := CstrCount - 1 downto 0 do
      for r := CstrList[c].VarRefN - 1 downto 0 do
      begin
        vi := 0;
